...
class BBIO:
	...
	def raw_cfg_pins(self, config):
		self.port.write(chr(0x40 | config).encode('utf-8'))

		return self.response(1)

	def raw_set_pins(self, pins):
		self.port.write(chr(0x80 | pins).encode('utf-8'))
	...
	def setup_PWM(self, prescaler, dutycycle, period):
		...
		self.port.write(chr(prescaler).encode('utf-8'))
		self.port.write(chr((dutycycle >> 8) & 0xFF).encode('utf-8'))
		self.port.write(chr(dutycycle & 0xFF).encode('utf-8'))
		self.port.write(chr((period >> 8) & 0xFF).encode('utf-8'))
		self.port.write(chr(period & 0xFF).encode('utf-8'))
	...
	def bulk_trans(self, input):
		...
		self.port.write(chr(0x10 | (byteCount - 1)).encode('utf-8'))

		#for i in range(byteCount):
		#	self.port.write(chr(byteList[i]).encode('utf-8'))
		self.port.write(byteList)

		data = self.response(byteCount + 1, True)
		return data[1:].decode('utf-8')

	def cfg_pins(self, pins=0):
		self.port.write(chr(0x40 | pins).encode('utf-8'))
	...
	def set_speed(self, spi_speed=0):
		self.port.write(chr(0x60 | spi_speed).encode('utf-8'))
