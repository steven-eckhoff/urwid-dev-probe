...
class I2Chigh(I2C):
	...
	def get_byte(self, i2caddr, addr):
		...
		stat = self.bulk_trans([i2caddr << 1, addr])
		self.send_start_bit()
		#stat += self.bulk_trans(1, [i2caddr << 1 | 1])
		stat += self.bulk_trans([i2caddr << 1 | 1])
		r = self.read_byte()
		self.send_nack()
		self.send_stop_bit()
	...
	def set_byte(self, i2caddr, addr, value):
		...
		stat = self.bulk_trans([i2caddr << 1, addr, value])
		self.send_stop_bit()
		if stat.find(chr(0x01)) != -1:
			raise IOError("I2C command on address 0x%02x not acknowledged!" % (i2caddr))
	...
	def command(self, i2caddr, cmd):
		...
		stat = self.bulk_trans([i2caddr << 1, cmd])
		self.send_stop_bit()
		if stat[0] == chr(0x01):
			raise IOError("I2C command on address 0x%02x not acknowledged!" % (i2caddr))
	...
	def set_word(self, i2caddr, addr, value):
		...
		vl = value % 256
		self.send_start_bit()
		#stat = self.bulk_trans([i2caddr << 1, addr, vh, vl])
		stat = self.bulk_trans([i2caddr << 1, addr, vh, vl])
		self.send_stop_bit()
		if stat.find(chr(0x01)) != -1:
			raise IOError("I2C command on address 0x%02x not acknowledged!" % (i2caddr))
